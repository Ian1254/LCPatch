...
private const val AppMotionDurationMs = 340
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            MiuixTheme(controller = controller) {
                App(themeMode) { value ->
                    themeMode = value
                    appPrefs.edit().putString("theme_mode", value).apply()
                }
            }
        }
    }
    ...
    private fun App(themeMode: String, onThemeMode: (String) -> Unit) {
        ...
        var page by rememberSaveable {
            mutableIntStateOf(if (appPrefs.getBoolean("onboarding_done", false)) OVERVIEW else ONBOARDING)
        }
        ...
        val pagerPosition = (pagerState.currentPage + pagerState.currentPageOffsetFraction).coerceIn(0f, 2f)
        ...
        var navigationStyle by remember {
            mutableStateOf(appPrefs.getString("navigation_style", "floating") ?: "floating")
        }
        var blurEnabled by remember { mutableStateOf(appPrefs.getBoolean("blur_enabled", true)) }
        var updateChannel by remember {
            mutableStateOf(appPrefs.getString("update_channel", "stable") ?: "stable")
        }
        ...
        LaunchedEffect(page) {
            ...
            if (target >= 0 && pagerState.currentPage != target) {
                pagerState.animateScrollToPage(
                    page = target,
                    animationSpec = tween(AppMotionDurationMs, easing = PageTransitionEasing)
                )
            }
        }
        ...
        fun checkRootStatus() {
            ...
            scope.launch {
                ...
                message = if (granted) "Root 權限已授予"
                else "未取得 Root 權限，套用漢化時將無法寫入遊戲資料"
            }
        }
                    ...
                    LazyColumn(
                        ...
                        when (visiblePage) {
                                    ...
                                    scope.launch {
                                        runCatching { translations.setTranslationEnabled(enabled) }
                                            .onSuccess {
                                                ...
                                                message = "漢化已${if (enabled) "啟用" else "停用"}，請重新啟動遊戲"
                                            }
                            ...
                            SETTINGS -> settings(
                                ...
                                onClearFont = {
                                    translations.clearCustomFont()
                                    fontName = translations.fontName()
                                    message = "已改回漢化包內字體"
                                },
                                onImport = {
                                    customPackPicker.launch(arrayOf("application/zip", "application/octet-stream"))
                                },
                                onReset = {
                                    logs.resetSelectedFolder()
                                    revision++
                                    message = "已改回預設暫存位置"
                                }
                            )
                            LOGS -> logPage(
                                ...
                                save = {
                                    try {
                                        message = "診斷文件已儲存：${logs.saveDiagnostic(events).lastPathSegment}"
                                    } catch (error: Throwable) {
                                ...
                                share = {
                                    try {
                                        startActivity(logs.shareIntent(events))
                                    } catch (error: Throwable) {
                            ...
                            DISPLAY -> displaySettings(
                                ...
                                onNavigationStyle = {
                                    navigationStyle = it
                                    appPrefs.edit().putString("navigation_style", it).apply()
                                },
                                blurEnabled = blurEnabled,
                                onBlurEnabled = { enabled ->
                                    blurEnabled = enabled
                                    appPrefs.edit().putBoolean("blur_enabled", enabled).apply()
                                },
                            ...
                            DOWNLOADED -> downloadedPage(
                                ...
                                convert = { pack, conversion ->
                                    if (!applying) {
                                        ...
                                        taskState.launchTask {
                                            runCatching {
                                                translations.convertDownloaded(pack, conversion) { value ->
                                                    withContext(Dispatchers.Main.immediate) { applyProgress = value }
                                                }
                                            }.onSuccess { result ->
                                ...
                                if (!applying) {
                                    ...
                                    taskState.launchTask {
                                        runCatching {
                                            translations.apply(pack) { value ->
                                                withContext(Dispatchers.Main.immediate) { applyProgress = value }
                                            }
                                        }.onSuccess {
                                            activeName = it
                                            activeScript = translations.activeScript()
                                            message = "已套用 $it，請重新啟動遊戲"
                                        }.onFailure {
                                            message = "套用失敗：${it.message}"
                                        }
                                        applying = false
        ...
        AnimatedContent(
            ...
            transitionSpec = {
                ...
                slideInHorizontally(tween(AppMotionDurationMs, easing = PageTransitionEasing)) {
                    direction * it
                } togetherWith slideOutHorizontally(
                    tween(AppMotionDurationMs, easing = PageTransitionEasing)
                ) {
                    -direction * it
                }
            },
                ...
                Scaffold(
                    ...
                    Box(
                        modifier = Modifier.fillMaxSize().then(
                            if (activeBarBackdrop != null) Modifier.layerBackdrop(activeBarBackdrop)
                            else Modifier
                        )
                    ) {
                        if (topLevelScreen) {
    ...
    private fun LazyListScope.overview(
        ...
        onTranslationEnabled: (Boolean) -> Unit,
        targetLanguage: OverrideLanguage,
        runtimeInspection: RuntimeInspection,
        onDownload: () -> Unit,
        onDownloaded: () -> Unit
    ) {
        ...
        item {
            Card(insideMargin = PaddingValues(18.dp)) {
                ...
                Text(
                    ...
                    color = if (runtimeInspection.ready && runtimeInspection.fontReady) {
    ...
    private fun LazyListScope.settings(
        ...
        applying: Boolean,
        fontName: String,
        onLanguage: (OverrideLanguage) -> Unit,
        onFont: () -> Unit,
        onClearFont: () -> Unit,
        onImport: () -> Unit,
        onReset: () -> Unit
            ...
            Card {
                OverlayDropdownPreference(
                    ...
                    onSelectedIndexChange = { index ->
                        TranslationRepository.LANGUAGES.getOrNull(index)?.let(onLanguage)
                    }
                )
                ArrowPreference(
                    modifier = PreferenceItemModifier,
                    title = "繁簡轉換",
                    summary = "轉換已下載漢化目錄內的 JSON 文件",
                    onClick = onConversion
                )
                ArrowPreference(
                    modifier = PreferenceItemModifier,
                    title = "匯入自訂漢化文件",
                    summary = "從本機加入 ZIP 漢化包",
                    onClick = onImport
                )
                ArrowPreference(
                    modifier = PreferenceItemModifier,
        ...
        item {
            Card {
                ...
                ArrowPreference(
                    modifier = PreferenceItemModifier,
                    title = "診斷文件儲存位置",
                    summary = logs.selectedFolderLabel(),
                    onClick = onFolder
                )
                ArrowPreference(
                    modifier = PreferenceItemModifier,
    ...
    private fun LazyListScope.displaySettings(
        ...
        item {
            Card {
                ...
                OverlayDropdownPreference(
                    ...
                    onSelectedIndexChange = {
                        onNavigationStyle(if (it == 1) "floating" else "standard")
                    }
                )
                SwitchPreference(
                    modifier = PreferenceItemModifier,
                    title = "背景模糊",
                    summary = if (blurEnabled) "已啟用；頂欄與底欄保持即時模糊"
                    else "已停用；使用穩定的半透明背景",
                    checked = blurEnabled,
        ...
        item {
            InfoCard(
                "顯示效果",
                "可選擇使用背景模糊；關閉後頂欄與底欄會改用穩定的半透明背景。"
            )
        }
    }

    private fun LazyListScope.logPage(
        events: List<LogEvent>,
        save: () -> Unit,
        share: () -> Unit,
        clear: () -> Unit
        ...
        item {
            Card(insideMargin = PaddingValues(16.dp)) {
                Button(modifier = Modifier.fillMaxWidth(), onClick = share) {
                    Text("分享診斷文件")
                }
                Spacer(Modifier.height(8.dp))
                Row(
        ...
        if (events.isEmpty()) {
            item { InfoCard("尚無日誌", "啟用模組並啟動一次遊戲後，再回到這裡匯出診斷文件。") }
    ...
    private fun LazyListScope.aboutPage(game: GameInfo) {
        ...
        item {
            Row(
                ...
                Card(
                    modifier = Modifier.weight(1f),
                    insideMargin = PaddingValues(18.dp),
                    colors = translucentAboutCardColors()
                ) {
                    Text(
                        "LCPatch 版本",
                        color = MiuixTheme.colorScheme.onSurfaceVariantSummary,
        ...
        item {
            InfoCard(
                "關於 LCPatch",
                "LCPatch 用於管理社群與自訂漢化、字體以及語言覆蓋設定。遊戲更新後會先驗證目標結構，配置不相符時停止載入，以降低閃退風險。",
                translucent = true
            )
        }
        item {
            InfoCard(
                "開放原始碼與致謝",
                "介面採用 compose-miuix-ui，LSPosed 整合採用 libxposed API 102，繁簡轉換採用 opencc4j。漢化內容與授權條款歸各翻譯組及原作者所有。",
                translucent = true
            )
        }
    }
    ...
    private fun LazyListScope.onboardingPage(
        ...
        item {
            InfoCard(
                "歡迎使用 LCPatch",
                "完成模組作用域與 Root 權限設定後，即可在 App 內下載並套用漢化。"
            )
        }
        item {
            Card(insideMargin = PaddingValues(18.dp)) {
                ...
                Spacer(Modifier.height(6.dp))
                Text(
                    "檢查 LCPatch 模組與 Limbus Company 作用域，不會跳轉至其他 App。\n目前狀態：$scopeStatus",
                    color = MiuixTheme.colorScheme.onSurfaceVariantSummary
                )
                Spacer(Modifier.height(14.dp))
        ...
        item {
            Card(insideMargin = PaddingValues(18.dp)) {
                ...
                Spacer(Modifier.height(6.dp))
                Text(
                    "檢查 LCPatch 是否能使用 Root 權限，不會開啟其他 App。\n目前狀態：$rootStatus",
                    color = MiuixTheme.colorScheme.onSurfaceVariantSummary
                )
                Spacer(Modifier.height(14.dp))
                Button(
                    ...
                    Text(if (rootStatus == "正在請求") "正在檢查…" else "檢查 Root 權限")
                }
            }
        }
        item {
            ...
            Spacer(Modifier.height(8.dp))
            Text(
                "若模組尚未回報狀態，可先完成設定；重新啟動遊戲後，概觀頁會自動更新。",
                color = MiuixTheme.colorScheme.onSurfaceVariantSummary,
                fontSize = 13.sp
            )
        }
    ...
    @Composable
    private fun InfoCard(title: String, body: String, translucent: Boolean = false) {
        Card(
            insideMargin = PaddingValues(18.dp),
            ...
            Spacer(Modifier.height(6.dp))
            Text(body, color = MiuixTheme.colorScheme.onSurfaceVariantSummary)
        }
    }

    @Composable
    private fun translucentAboutCardColors() = CardDefaults.defaultColors(
        color = MiuixTheme.colorScheme.surfaceContainer.copy(alpha = 0.80f)
    )
    ...
    @Composable
    private fun Modifier.aboutShimmer(): Modifier {
        ...
        val glow = MiuixTheme.colorScheme.primary.copy(alpha = 0.34f)
        val secondary = MiuixTheme.colorScheme.secondaryContainer.copy(alpha = 0.30f)
        return drawBehind {
            drawRect(
                brush = Brush.verticalGradient(
                    colors = listOf(
                        glow.copy(alpha = glow.alpha * 0.72f),
                        secondary.copy(alpha = secondary.alpha * 0.56f),
            ...
            drawRect(
                brush = Brush.linearGradient(
                    colors = listOf(
                        ...
                        secondary,
                        glow,
                        secondary.copy(alpha = secondary.alpha * 0.36f),
                        Color.Transparent
                    ),
    ...
    @Composable
    private fun LogCard(event: LogEvent) {
        Card(insideMargin = PaddingValues(16.dp)) {
            Text(
                "${event.level} · ${event.code}",
                fontWeight = FontWeight.SemiBold,
                color = when (event.level) {
                    "ERROR" -> MiuixTheme.colorScheme.error
                    "WARN" -> MiuixTheme.colorScheme.primaryVariant
            ...
            Spacer(Modifier.height(6.dp))
            Text(
                "${event.time}${if (event.process.isBlank()) "" else " · ${event.process}"}",
                fontSize = 12.sp,
                color = MiuixTheme.colorScheme.onSurfaceVariantSummary
            )
        }
    ...
    private fun launchGame() {
        Thread({
            ...
            runOnUiThread {
                try {
                    startActivity(
                        packageManager.getLaunchIntentForPackage(LogProvider.GAME)
                            ?: throw ActivityNotFoundException()
                    )
                } catch (_: ActivityNotFoundException) {
                    LogRepository.append(this, "ERROR", "game.missing", "找不到遊戲啟動入口")
                }
            }
    ...
    private fun LazyListScope.downloadPage(
            ...
            Card(insideMargin = PaddingValues(18.dp)) {
                ...
                Text(
                    when {
                        loading -> "正在取得可用漢化…"
                        error != null -> error
                        entries.isEmpty() -> "目前沒有可用項目"
                        else -> "已找到 ${entries.size} 個漢化版本"
                    },
                    color = MiuixTheme.colorScheme.onSurfaceVariantSummary
                )
                if (!loading) {
        ...
        transfer?.let { value ->
            item {
                Card(insideMargin = PaddingValues(18.dp)) {
                    val fraction = if (value.total > 0) {
                        (value.bytes.toFloat() / value.total).coerceIn(0f, 1f)
                    } else null
                    Text(
                        if (applying) "正在套用 ${value.name}"
                        else if (value.finished) "下載完成，準備套用"
                        else "正在下載 ${value.name}",
                    ...
                    Text(
                        if (value.finished) {
                            ...
                            "${if (fraction != null) "${(fraction * 100).toInt()}% · " else ""}${formatBytes(value.bytes)}${if (value.total > 0) " / ${formatBytes(value.total)}" else ""} · ${formatBytes(value.bytesPerSecond)}/s"
                        },
                        color = MiuixTheme.colorScheme.onSurfaceVariantSummary
                    )
                }
        ...
        items(entries) { entry ->
            Card(insideMargin = PaddingValues(18.dp)) {
                ...
                Spacer(Modifier.height(4.dp))
                Text("${entry.author} · ${entry.section}", color = MiuixTheme.colorScheme.primary)
                Spacer(Modifier.height(6.dp))
                Text(entry.description, color = MiuixTheme.colorScheme.onSurfaceVariantSummary)
                Spacer(Modifier.height(12.dp))
                Button(
                    modifier = Modifier.fillMaxWidth(),
                    enabled = transfer?.finished != false && !applying,
    ...
    private fun LazyListScope.downloadedPage(
        ...
        if (entries.isEmpty()) {
            item {
                InfoCard(
                    "尚無已下載漢化",
                    "請先從「下載漢化」取得漢化包。下載完成後會預設立即套用，也會保留在此供日後切換。"
                )
            }
        } else {
            ...
            items(entries) { entry ->
                ...
                LaunchedEffect(entry.script) {
                    conversionIndex = if (entry.script == "簡體") 1 else 0
                }
                Card(
                    insideMargin = PaddingValues(18.dp),
                    ...
                    Text(
                        entry.name,
                        style = MiuixTheme.textStyles.title2,
                        color = if (active) MiuixTheme.colorScheme.primary else Color.Unspecified
                    )
                    Spacer(Modifier.height(5.dp))
                    Text(
                    ...
                    OverlayDropdownPreference(
                        ...
                        onSelectedIndexChange = { index ->
                            ...
                            TranslationRepository.CONVERSIONS.getOrNull(index)?.let {
                                convert(entry, it)
                            }
                        }
                    )
                    if (processingPackPath == entry.path) {
                        applyProgress?.let { value ->
                            Spacer(Modifier.height(8.dp))
                            Text(
                                "${value.stage}：${entry.name}",
                                color = MiuixTheme.colorScheme.primary,
                                fontWeight = FontWeight.Medium
                            )
                            Spacer(Modifier.height(7.dp))
                            ...
                            Spacer(Modifier.height(5.dp))
                            Text(
                                if (value.total > 0) "已處理 ${value.current} / ${value.total} 個 JSON"
                                else "正在掃描此漢化目錄…",
                                color = MiuixTheme.colorScheme.onSurfaceVariantSummary,
                                fontSize = 13.sp
                            )
                    ...
                    Text(
                        ...
                        color = if (entry.puaPrepared) MiuixTheme.colorScheme.primary
                        else MiuixTheme.colorScheme.onSurfaceVariantSummary,
                        fontSize = 13.sp,
    ...
    private fun LazyListScope.conversionPage(
        ...
        if (entries.isEmpty()) {
            item {
                InfoCard(
                    "尚無可轉換的漢化",
                    "請先下載或匯入漢化。下載完成後，漢化會先解壓到 /sdcard/LCPatch/漢化 的獨立目錄。"
                )
            }
        } else {
            item {
                InfoCard(
                    "選擇漢化目錄",
                    "只會轉換你選中的已下載目錄；若它正是目前套用的漢化，完成後會自動重新套用。"
                )
            }
            items(entries) { entry ->
